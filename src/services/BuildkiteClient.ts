...
// Note: Normally these types would be generated by GraphQL Codegen
// Since we're having issues with the code generation, we're defining interfaces manually
// based on the expected GraphQL responses

// Manually defining the type interfaces
...
interface GetPipelinesQueryVariables {
  organizationSlug: string;
  first?: number;
  after?: string;
}

interface GetPipelinesQuery {
  organization: {
    pipelines: {
      edges: Array<{
        node: {
...
interface GetBuildsQueryVariables {
  pipelineSlug: string;
  organizationSlug: string;
  first?: number;
}

interface GetBuildsQuery {
  organization: {
    pipelines: {
      edges: Array<{
        node: {
...
interface GetViewerBuildsQueryVariables {
  first: number;
}

interface GetViewerBuildsQuery {
  viewer: {
    builds: {
      edges: Array<{
        node: {
...
export class BuildkiteClient {
  ...
  public async getPipelinesTyped(
    ...
    const startTime = process.hrtime.bigint();
    const result = await this.client.request<GetPipelinesQuery>(GET_PIPELINES.toString(), variables as any);
  ...
  public async getBuildsTyped(
    ...
    const startTime = process.hrtime.bigint();
    const result = await this.client.request<GetBuildsQuery>(GET_BUILDS.toString(), variables as any);
  ...
  public async getViewerBuildsTyped(first: number): Promise<GetViewerBuildsQuery> {
    ...
    const startTime = process.hrtime.bigint();
    const result = await this.client.request<GetViewerBuildsQuery>(GET_VIEWER_BUILDS.toString(), variables as any);
